...
// Copyright (c) 2014-2025 The BobMemeCoin Core developers
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (m_chain_helper.IsInstantSendWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in BobMemeCoin
...
static std::pair<CAmount, CAmount> GetBlockSubsidyHelper(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fV20Active)
{
    (void)nPrevBits;
    (void)fV20Active;
    static constexpr CAmount kBaseSubsidy = 5000 * COIN;
    const int nHalvingInterval = std::max(1, consensusParams.nSubsidyHalvingInterval);
    ...
    while (nRemainingRewardedBlocks > 0 && nEraSubsidy > 0) {
        ...
        nIssued += CAmount{nEraBlocks} * nEraSubsidy;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const ChainstateManager& chainman)
{
    ...
    // It always active on BobMemeCoin chains
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, view, fJustCheck, fScriptChecks, state, mnlist_updates_opt)) {
        return error("ConnectBlock(BOBMEMECOIN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    if (m_chain_helper->ShouldInstantSendRejectConflicts()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (auto conflictLockOpt = m_chain_helper->ConflictingISLockIfAny(*tx)) {
                ...
                if (has_chainlock) {
                    LogPrint(BCLog::ALL, "ConnectBlock(BOBMEMECOIN): chain-locked transaction %s overrides islock %s\n", tx->GetHash().ToString(), conflict_islock_hash.ToString());
                    m_chain_helper->RemoveConflictingISLockByTx(*tx);
                } else {
                    // The node which relayed this should switch to correct chain.
                    // TODO: relay instantsend data/proof.
                    LogPrintf("ERROR: ConnectBlock(BOBMEMECOIN): transaction %s conflicts with transaction lock %s\n", tx->GetHash().ToString(), conflict_txid.ToString());
    ...
    if (!m_chain_helper->mn_payments->IsBlockValueValid(block, pindex->nHeight, blockSubsidy + feeReward, strError, check_superblock)) {
        ...
        LogPrintf("ERROR: ConnectBlock(BOBMEMECOIN): %s\n", strError);
    ...
    if (!m_chain_helper->mn_payments->IsBlockPayeeValid(*block.vtx[0], pindex->pprev, blockSubsidy, feeReward, check_superblock)) {
        // NOTE: Do not punish, the node might be missing governance data
        LogPrintf("ERROR: ConnectBlock(BOBMEMECOIN): couldn't find masternode or superblock payments\n");
    ...
    TRACE6(validation, block_connected,
        ...
        nInputs,
        nSigOps,
        nTime8 - nTimeStart // in microseconds (Вµs)
    );
...
CVerifyDB::CVerifyDB()
{
    uiInterface.ShowProgress(_("Verifying blocksвЂ¦").translated, 0, false);
}
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        uiInterface.ShowProgress(_("Verifying blocksвЂ¦").translated, percentageDone, false);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, inputs, false /*fJustCheck*/, false /*fScriptChecks*/, state, mnlist_updates_opt)) {
        return error("RollforwardBlock(BOBMEMECOIN): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (fAddressIndex) {
        if (!m_blockman.m_block_tree_db->WriteAddressIndex(addressIndex)) {
            return error("RollforwardBlock(BOBMEMECOIN): Failed to write address index");
        }

        if (!m_blockman.m_block_tree_db->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("RollforwardBlock(BOBMEMECOIN): Failed to write address unspent index");
        }
    }

    if (fSpentIndex) {
        if (!m_blockman.m_block_tree_db->UpdateSpentIndex(spentIndex))
            return error("RollforwardBlock(BOBMEMECOIN): Failed to write transaction index");
    }

    if (fTimestampIndex) {
        if (!m_blockman.m_block_tree_db->WriteTimestampIndex(CTimestampIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return error("RollforwardBlock(BOBMEMECOIN): Failed to write timestamp index");
    }
...
bool CChainState::ReplayBlocks()
{
    ...
    uiInterface.ShowProgress(_("Replaying blocksвЂ¦").translated, 0, false);
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        if (fDIP0003Active && !m_evoDb.VerifyBestBlock(pindexOld->GetBlockHash())) {
            return error("ReplayBlocks(BOBMEMECOIN): Found EvoDB inconsistency");
        }
    }
    ...
    for (int nHeight = nForkHeight + 1; nHeight <= pindexNew->nHeight; ++nHeight) {
        ...
        uiInterface.ShowProgress(_("Replaying blocksвЂ¦").translated, (int) ((nHeight - nForkHeight) * 100.0 / (pindexNew->nHeight - nForkHeight)) , false);
                                            ...
                                            std::optional<bool> optHaveDIP0024Quorums) const
{
    ...
    // BobMemeCoin small-network quorums: enabled after nSMTSmallQuorumsHeight (block 45,000)
