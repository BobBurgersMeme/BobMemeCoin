    ...
    m_wallet_model(wallet_model)
{
    ...
    auto* intro_text = new QLabel(tr("This wizard helps you create and register a masternode without typing long manual commands.\n\n"
                                     ...
                                     "3. Save operator key into bobmemecoin.conf\n"
    ...
    m_mn_type->addItem(tr("Regular (15,000 BMC)"), static_cast<int>(MnType::Regular));
    m_mn_type->addItem(tr("Evo (75,000 BMC)"), static_cast<int>(MnType::Evo));
    ...
    auto* bls_text = new QLabel(tr("Generate operator key pair. The secret key will be saved to bobmemecoin.conf as masternodeblsprivkey."));
...
bool MasternodeSetupWizard::validateInput(QString& error) const
{
    ...
    auto require_valid_address = [this, &error](const QLineEdit* field, const QString& label) {
        ...
        if (!m_wallet_model->validateAddress(value)) {
            error = tr("%1 is not a valid BobMemeCoin address.").arg(label);
            return false;
        }
        return true;
    };
...
bool MasternodeSetupWizard::saveOperatorSecretToConfig(QString& error)
{
    ...
    if (config_path.empty()) {
        error = tr("Unable to resolve bobmemecoin.conf path.");
        return false;
    }
...
void MasternodeSetupWizard::updateTypeUi()
{
    ...
    m_collateral_label->setText(evo ? tr("75000 BMC") : tr("15000 BMC"));
...
void MasternodeSetupWizard::updateSummary()
{
    ...
    lines << tr("Collateral: %1").arg(evo ? tr("75000 BMC") : tr("15000 BMC"));
...
void MasternodeSetupWizard::accept()
{
    ...
    if (m_restart_required) {
        success += tr("\n\nRestart BobMemeCoin Core so local masternode service uses the new key.");
    }
