...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BobMemeCoin address (e.g. %1)").arg(
...
{
    // Return if URI is not valid or is not a bobmemecoin: URI.
    if(!uri.isValid() || uri.scheme() != QString("bobmemecoin"))
        return false;
...
{
    QString ret = QString("bobmemecoin:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BobMemeCoin Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BobMemeCoin Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BobMemeCoin Core (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bobmemecoincore.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bobmemecoincore-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BobMemeCoin Core\n";
        else
            optionFile << strprintf("Name=BobMemeCoin Core (%s)\n", chain);
