...
static RPCHelpMan listaddressbalances()
{
    return RPCHelpMan{"listaddressbalances",
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::STR_AMOUNT, "amount", "The BobMemeCoin address and the amount in " + CURRENCY_UNIT},
            }
        },
        RPCExamples{
            ...
            if (!pwallet->Unlock(wallet_passphrase)) {
                ...
                if (wallet_passphrase.find('\0') == std::string::npos) {
                    ...
                    throw JSONRPCError(RPC_WALLET_PASSPHRASE_INCORRECT, "Error: The wallet passphrase entered is incorrect. "
                                                                        ...
                                                                        "please try again with only the characters up to вЂ” but not including вЂ” "
...
static RPCHelpMan loadwallet()
{
    return RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting bobmemecoind will be"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[5].isNull() && request.params[5].get_bool()) {
        ...
        if (request.params[6].isNull()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "The createwallet RPC requires specifying the 'load_on_startup' flag when creating descriptor wallets. BobMemeCoin Core v21 introduced this requirement due to breaking changes in the createwallet RPC.");
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    pwallet->ShowProgress(strprintf("%s " + _("Wiping wallet transactionsвЂ¦").translated, pwallet->GetDisplayName()), 0);
    ...
    for (size_t progress = 0; progress < STEPS; ++progress) {
        ...
        if (vHashIn.size() > 0 && pwallet->ZapSelectTx(vHashIn, vHashOut) != DBErrors::LOAD_OK) {
            pwallet->ShowProgress(strprintf("%s " + _("Wiping wallet transactionsвЂ¦").translated, pwallet->GetDisplayName()), 100);
        ...
        pwallet->ShowProgress(strprintf("%s " + _("Wiping wallet transactionsвЂ¦").translated, pwallet->GetDisplayName()), std::max(1, std::min(99, int(progress * 100 / STEPS))));
    }
