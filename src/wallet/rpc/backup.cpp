...
RPCHelpMan importaddress()
{
    return RPCHelpMan{"importaddress",
            ...
            "Note: This command is only compatible with legacy wallets. Use \"importdescriptors\" with \"addr(X)\" for descriptor wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BobMemeCoin address (or hex-encoded script)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BobMemeCoin address or script");
        }
    }
    if (fRescan)
    {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fGood = true;
    {
        ...
        pwallet->chain().showProgress(strprintf("%s " + _("ImportingвЂ¦").translated, pwallet->GetDisplayName()), 0, false); // show progress dialog in GUI
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    pwallet->ShowProgress(_("ImportingвЂ¦").translated, 0); // show progress dialog in GUI
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                "Note: This command is only compatible with legacy wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BobMemeCoin address for the private key"},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("bobmemecoincoreversion", FormatFullVersion());
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see bobmemecoind log for details) and could "
