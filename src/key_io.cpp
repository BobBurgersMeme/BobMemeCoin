...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str)
{
    ...
    uint160 hash;
    error_str = "";
    const auto max_prefix_len = std::max(params.Base58Prefix(CChainParams::PUBKEY_ADDRESS).size(),
                                         params.Base58Prefix(CChainParams::SCRIPT_ADDRESS).size());
    if (DecodeBase58Check(str, data, static_cast<int>(hash.size() + max_prefix_len))) {
