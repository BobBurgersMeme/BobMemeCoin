...
// Copyright (c) 2014-2026 The BobMemeCoin Core developers
 ...
 */
static const char* BITCOIN_PID_FILENAME = "bobmemecoind.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-sporkaddr=<bobmemecoinaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-deprecated-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by BobMemeCoin Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
{
    // Make sure only a single BobMemeCoin Core process is using the data directory.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if BobMemeCoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if BobMemeCoin Core is started while in a temporary directory.\n",
...

    {
        ...
        // Initialize addrman
        assert(!node.addrman);
        uiInterface.InitMessage(_("Loading P2P addressesвЂ¦").translated);
    ...
    for (bool fLoaded = false; !fLoaded && !ShutdownRequested();) {
        ...
        uiInterface.InitMessage(_("Loading block indexвЂ¦").translated);
        ...
        if (maybe_load_error.has_value()) {
            ...
            try {
                uiInterface.InitMessage(_("Verifying blocksвЂ¦").translated);
    ...
    // ********************************************************* Step 7d: Setup other BobMemeCoin services
    ...
    if (fPruneMode) {
        if (!fReindex) {
            ...
            for (CChainState* chainstate : chainman.GetAll()) {
                uiInterface.InitMessage(_("Pruning blockstoreвЂ¦").translated);
    ...
    // ********************************************************* Step 10a: schedule BobMemeCoin-specific tasks
